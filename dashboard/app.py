...
def create_app():
    ...
    def api_dashboard_data():
        ...
        # Peer volume analysis + bank comparison
        from services.analytics import (get_peer_yoy_comparison, get_complaint_drivers_yoy,
        ...
        date_range = db.session.query(
            db.func.min(Complaint.date_received), db.func.max(Complaint.date_received)).first()
        data_sources = {
            'cfpb': {'status': 'active', 'complaints': complaint_count, 'companies': company_count,
            ...
            'fdic': {'status': 'active', 'description': 'Bank financials via live API'},
            'sec_edgar': {'status': 'active', 'description': 'Company filings via live API'},
            'finra': {'status': 'limited', 'description': 'Broker disclosures via search API'},
        }
    ...
    @login_required
    def api_warm_cache():
        """Pre-warm caches for top companies. Called once on page load."""
        import time as _t
        from services.analytics import get_companies
        ...
        warmed = []
        for c in companies:
            ck = f'{c["company"]}_{months_back}'.replace(' ', '_').replace(',', '')
            # Skip if already cached
            if ck in _dashboard_cache and _t.time() - _dashboard_cache[ck][1] < 300:
                continue
            fd = _load_file_cache(ck)
            if fd:
                _dashboard_cache[ck] = (fd, _t.time())
                continue
            warmed.append(c['company'])
        # Return which ones need warming - frontend will request them
        return jsonify({'need_warming': warmed[:5]})
